...
############################################################################### 
# Define arguments   
parser = argparse.ArgumentParser(description=' This programs aims to merge land mascons left alone with the closest appropriate mascon')
...
if outputfile=="None":
      outputfile=inputfile+'_lfixed'
 
print('')
print('Merge solo land mascons with closest primary:',str(datetime.datetime.now())[0:19])
...
print('')
print('Select solo land ternaries: %s'%(str(datetime.datetime.now())[0:19])) 
outTdata=Tdata[Tdata[:,13]=='Land'] # ternaries inside polygons
nbt_out=len(outTdata[:,0])
print('Found %d land ternary mascons to merge with other primaries: %s'%(nbt_out,str(datetime.datetime.now())[0:19])) 
...
for tpt in np.arange(nbt_out):
    ...
    print('Land ternary %d (%6.2f,%6.2f) merged with primary %d in %s. Timetag: %s'%(tpd[tpt,0],outTdata[tpt,2],outTdata[tpt,3],tpd[tpt,1],tselect[index,13],str(datetime.datetime.now())[0:19]))  
...
lastcomment='# fix_solo_land_mascons.py : merge solo land mascons with appropriate primary. Timetag: %s \n'%(str(datetime.datetime.now())[0:19])
