...
def main():
    ...
    for cpt in np.arange(nbr):
        try:
            ...
            sfile.write("Error in ~/gt/util/pycodes/pyshape_constrained_size_mascons.py %s %d --overflow=%d --precision=%d --maxsize=%d --region=%s --out_masconfile=%s. Timetag: %s"%(tempfile,area,overflow,precision,maxsize,regions[cpt],tempfile,str(datetime.datetime.now())[0:19]))
        else:
            sfile.write("~/gt/util/pycodes/pyshape_constrained_size_mascons.py %s %d --overflow=%d --precision=%d --maxsize=%d --region=%s --out_masconfile=%s. Timetag: %s"%(tempfile,area,overflow,precision,maxsize,regions[cpt],tempfile,str(datetime.datetime.now())[0:19]))
    ...
    try:
        ...
        sfile.write("Error in ~/gt/util/pycodes/merge_small_primaries.py %s --outfile=%s --minsize=25 --maxdist=750 --ftype=1. Timetag: %s."%(tempfile,tempfile,str(datetime.datetime.now())[0:19]))
    else:
        sfile.write("~/gt/util/pycodes/merge_small_primaries.py %s --outfile=%s --minsize=25 --maxdist=750 --ftype=1. Timetag: %s."%(tempfile,tempfile,str(datetime.datetime.now())[0:19]))
    ...
    try:
        ...
        sfile.write("Error in ~/gt/util/pycodes/merge_small_primaries.py %s --outfile=%s --minsize=%d --maxdist=%d --ftype=0. Timetag: %s."%(tempfile,tempfile,minsize,maxdist,str(datetime.datetime.now())[0:19]))
    else:
        sfile.write("~/gt/util/pycodes/merge_small_primaries.py %s --outfile=%s --minsize=%d --maxdist=%d --ftype=0. Timetag: %s."%(tempfile,tempfile,minsize,maxdist,str(datetime.datetime.now())[0:19]))
    ...
    try:
        ...
        sfile.write("Error in ~/gt/util/pycodes/reshape_large_primaries.py %s --outfile=%s --minarea=%d --maxarea=%d --area=%d. Timetag: %s."%(tempfile,tempfile,int(0.75*area),int(1.25*area),area,str(datetime.datetime.now())[0:19]))
    else:
        sfile.write("~/gt/util/pycodes/reshape_large_primaries.py %s --outfile=%s --minarea=%d --maxarea=%d --area=%d. Timetag: %s."%(tempfile,tempfile,int(0.75*area),int(1.25*area),area,str(datetime.datetime.now())[0:19]))
