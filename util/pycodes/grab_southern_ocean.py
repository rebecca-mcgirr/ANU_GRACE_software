...
############################################################################### 
# Define arguments   
parser = argparse.ArgumentParser(description=' This programs aims to create a Southern Ocean region (one primary mascon) including all ocean mascons below 55 degree south')
...
new_Pdata[pcpt+1,13]='SouthernO'# new region name defined in polygon file
for tcpt in np.arange(new_tip):
    ...
    new_Tdata[tcount,13]='SouthernO' # new region name defined in polygon file
del ternaries, new_tip, pcland    
...
lastcomment='# grab_southern_ocean.py : define southern ocean as all ocean mascons below 55 degree south. Timetag: %s \n'%(str(datetime.datetime.now())[0:19])
