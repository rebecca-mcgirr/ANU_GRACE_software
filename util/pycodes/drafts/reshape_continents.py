...
# Declare arguments
infile = '/scratch/compute1/julia/solutions/newcontinents/mascons_stage4_JP15'
area = 90000000000
outfile = '/scratch/compute1/julia/solutions/newcontinents/mascons_stage4_JP16'
maxsize = 1000000000000
overflow = 20
precision = 7500
### Read polygon file
_,Pdata,_,_=gr.read_mascon_file(infile)
regions=['Eurasia','Africa','NAmerica','SAmerica','Madagascar','Casp','England']
nbr=len(regions)
...
#os.system("cp %s %s"%(tempfile,'/scratch/compute1/julia/solutions/newcontinents/mascons_stage4_JP15b'))    
del regions
regions,coords,nbv=gr.read_polygons_ascii('/scratch/compute1/julia/solutions/newcontinents/riverbasins_grdc.polygons','all')
nbr=len(regions)
for cpt in np.arange(nbr):
    os.system("~/gt/util/pycodes/pyshape_constrained_size_mascons_RM.py %s %d --overflow=%d --precision=%d --maxsize=%d --region=%s --out_masconfile=%s"%(tempfile,area,overflow,precision,maxsize,regions[cpt],tempfile))
