...
def read_addnorm_fit(filename):
    ...
    ####
    mydate=re.search('\d{4}_\d{2}_\d{2}',lines[3][-45:]).group()
    ...
    pcount=0
    for cpt in np.arange(4+nbf+25*nbf,len(lines)):
        if lines[cpt]=='\n':
            pass
        else:
            params_msc[pcount,0]=float(lines[cpt].split()[-4])
            params_msc[pcount,1]=float(lines[cpt].split()[-3])
            params_msc[pcount,2]=float(lines[cpt].split()[-2])
            params_msc[pcount,3]=float(lines[cpt].split()[-1])
            pcount=pcount+1
    return monthly_date,params_msc,nbf
############################################################################## 
