    ...
    if nbdays>=7: # number of minimum daily solutions in a month
        ...
        if os.path.isfile(monthly_dir+'/addnorm_%s_%4d_%02d.fit'%(prefix_reg,mydate.year,mydate.month)):
            ...
            for cpt in np.arange(nbdays):
                ...
                if datadir[-1]=='/':
                    ofile.write('{:s}\n'.format(datadir+'msc%4d-%02d-%02d/msc_%4d_%02d_%02d_iter3.norm'%(mydate.year,mydate.month,mydate.day,mydate.year,mydate.month,mydate.day)))
                else:
                    ofile.write('{:s}\n'.format(datadir+'/msc%4d-%02d-%02d/msc_%4d_%02d_%02d_iter3.norm'%(mydate.year,mydate.month,mydate.day,mydate.year,mydate.month,mydate.day)))
            ofile.close() 
            os.chdir(monthly_dir)
            myprefix='addnorm_%s_%4d_%02d'%(prefix_reg,mydate.year,mydate.month)
            os.system("~/gt/gracefit_paul/addnorm addnorm.cmd gracefit.cmd %s %s"%(mascon_file,myprefix))
