...
for cpt in np.arange(NB):
    ...
    annual_amplitudes[cpt]=np.sqrt(coefs[0]**2+coefs[1]**2)
## Compute RMS
...
ofile.write('# Latitude \t Longitude \t annual_amplitude \t total_rms \t annual_rms \t lowpass_rms \t HF_rms  \t annual+HF_rms  \t lowpass_0 ... lowpass_%d\n'%(NT))
ofile.write('{:5d}\t{:5d}\t{:5d}\n'.format(NB,NT,6))
for tpt in np.arange(NT):
        ofile.write('{:10.5f}\t'.format(decyear[tpt])) 
ofile.write('\n')
for cpt in np.arange(NB):
    ofile.write('{:10.4f}\t{:10.4f}\t{:10.4f}\t{:10.4f}\t{:10.4f}\t{:10.4f}\t{:10.4f}\t{:10.4f}'.format(lat[cpt],lon[cpt],annual_amplitudes[cpt],rms_total[cpt],rms_annual[cpt],rms_lowpass[cpt],rms_highfreq[cpt],rms_hfan[cpt])) 
    for tpt in np.arange(NT):
