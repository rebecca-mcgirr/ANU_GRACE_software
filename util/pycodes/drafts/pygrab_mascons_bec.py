...
##############################################################################
    ...
    #elif myregion[:5]=='Antar':
        #Tmask=(iTdata[:,2]>=-60)&(iTdata[:,8]==mydensity) # defime mask with bounding box and density
...
#################################################################################
    ...
    for pcpt in np.arange(nbp_out,new_nbp):#primaries
        ...
        for tcpt in np.arange(new_tip):
            ...
            new_Tdata[tcount,11:13]=1600 # pcolour and scolour unchanged
...
#################################################################################        
    print('')
    print('Create map of new primaries (map_pygrab_mascons_%s.png):'%(myregion),str(datetime.datetime.now())[0:19])
    import matplotlib
    havedisplay = "DISPLAY" in os.environ
    if not havedisplay:
        matplotlib.use("Agg")
    ...
    _,Pdata,_,Tdata=gr.read_mascon_file(out_masconfile)
    regions,coords,nbv=gr.read_polygons_ascii(polygon_file,myregion)
    mybufferdist=0.25
    minlat,maxlat,minlon,maxlon=gr.boundingbox(coords,mybufferdist)
    if minlon<0:
    ...
    nbp=len(Pdata)
    id_primary=[]
    # search for primary whose region == polygon name
    for pcpt in np.arange(nbp):
        if Pdata[pcpt,13] in regions:
            id_primary.append(pcpt)
    nbpin=len(id_primary)
    inPdata=Pdata[id_primary]
    nbt=len(Tdata)
    id_ternary=[]
    # search for ternary whose region == polygon name
    for tcpt in np.arange(nbt):
        if Tdata[tcpt,13] in regions:
            id_ternary.append(tcpt)
    inTdata=Tdata[id_ternary]
    #### colorbar
    colors=[]
    cm20=plt.cm.Set1
    ...
    del Pdata,Tdata,inPdata,inTdata,id_primary,id_ternary,nbp,nbt,colors,cm20,Px,Py,Tx,Ty
    print('')
    print('#######################################################################################################')
