...
!           have to deal with it.
  ...
  integer*4     :: start_ep,end_ep    ! start and end epochs within which to fit the model
...
! temp variables for reading accelerometer values
  ...
  integer*4     :: dt                    ! seconds between time of a thrust and the graceseconds of the start of an orbit
...
! least squares variables
  ...
  real(kind=8)  :: sigmas(3,3)             ! formal sigmas of the parameter estimates
...
! parse command line arguments
  call getarg(1,arg)
  if(arg(1:1) == "")then
    call status_update('FATAL','UTIL','model_acc',' ' &
         ,"Runstring: model_acc 2010 09 07 poly_degree bit-mapped-value start_ep end_ep",0)
  else
    read(arg,*)yr
    call getarg(2,arg)
    read(arg,*)month
...
!print*,'isat,ithrust,dt,thr_data(ithrust,isat)',isat,ithrust,dt,thr_data(ithrust,isat)
        if(dt < 60)then
...
! the least squares. Solve for offset, rate and quadratic terms
  nparam = 3
! PT170801: define size of matrices based on user-defined start/end epochs
  nepochs = end_ep-start_ep + 1
...
! loop over satellites
  do isat = 1,2
    ...
    do iepoch = start_ep,end_ep
      if (acc_data(iepoch,4,isat) < 99.d0) then     ! it is a valid observation that is unaffected by thrusts
        nobs = nobs+1
      endif
    enddo
    allocate(A(nobs,nparam))
    allocate(At(nparam,nobs))
!We dont use it    allocate(W(nobs,nobs))
    allocate(AtW(nparam,nobs))
...
!if(mod(iepoch,100) == 0)print*,'iepoch:',iepoch
          if (acc_data(iepoch,4,isat) < 99.d0) then     ! it is a valid observation that is unaffected by thrusts
            ...
            B(nobs,1) = acc_data(nobs,axis,isat) - ( apriori(1,axis)*t**2 + apriori(2,axis)*t + apriori(3,axis) )       ! just do the cross-track for now ....
          else
! print*,'axis,iepoch,bad data',axis,iepoch
          endif
        enddo
        print*,'isat,axis, nobs',isat,axis,nobs

! least squares inversion
!      call status_update('STATUS','UTIL','model_acc',' ',"Least squares inversion: transpose A matrix",0)
        call transp(A,At,nobs,nparam)
        call matmult(At,A,AtWA,nparam,nobs,nparam)
...
!      call status_update('STATUS','UTIL','model_acc',' ',"Least squares inversion: AtWB",0)
        call matmult(At,B,AtWB,nparam,nobs,1)
        call matmult(VCV,AtWB,soln,nparam,nparam,1)

!        print*,'iter, isat, axis, xhat: ',iter,isat,axis,soln(:,1)
        apriori(:,axis) = apriori(:,axis) + soln(:,1)

! PT170801: save the sigmas of the parameters as well
        do i=1,3
          sigmas(i,axis) = dsqrt(VCV(i,i))/86400.d0
        enddo
!print*,'axis sigmas',axis,sigmas
! if(iter == 4)call printmat (VCV,3,3,"VCV ")
      enddo
    enddo
...
!           value (stored in "apriori") to zero if we don't want to update
    if( .not. bitmap(bit_map,1))then
      ...
      write(message,'(a,i3,3(a,f12.3,a,f18.3),a)')" Remove model from accelerometer obs for X-axis for sat: ",isat &
         ," accel:",apriori(1,1)*1.e9,"+-",sigmas(1,1)*1.e9  &
         ,", rate:",apriori(2,1)*1.e9,"+-",sigmas(1,2)*1.e9  &
         ,", bias:",apriori(3,1)*1.e9,"+-",sigmas(1,3)*1.e9," (nm/s^2)"
!      write(message,'(a,i3,3(a,f12.3),a)')" Remove model from accelerometer obs for X-axis for sat: ",isat &
!         ," accel:",apriori(1,1)*1.e9,", rate: ",apriori(2,1)*1.e9,", bias:",apriori(3,1)*1.e9," (nm/s^2)"
      call status_update('STATUS','UTIL','model_acc',' ',message,0)
    endif
    if(  .not. bitmap(bit_map,2))then
      apriori(:,2) = 0.d0
    else
      write(message,'(a,i3,3(a,f12.3),a)')" Remove model from accelerometer obs for Y-axis for sat: ",isat &
         ," accel:",apriori(1,2)*1.e9,", rate: ",apriori(2,2)*1.e9,", bias:",apriori(3,2)*1.e9," (nm/s^2)"
      call status_update('STATUS','UTIL','model_acc',' ',message,0)
    endif
    if(  .not. bitmap(bit_map,3))then
      apriori(:,3) = 0.d0
    else
      write(message,'(a,i3,3(a,f12.3),a)')" Remove model from accelerometer obs for Z-axis for sat: ",isat &
         ," accel:",apriori(1,3)*1.e9,", rate: ",apriori(2,3)*1.e9,", bias:",apriori(3,3)*1.e9," (nm/s^2)"
      call status_update('STATUS','UTIL','model_acc',' ',message,0)
    endif

! PT170801: here, leave the start/end epochs to be the entire day
    do iepoch = 1,86400
...
! output, correcting for only acceleration and rate (leave the offset bias in there .....)
        write(luacc_out(isat),*)start_gracesec(isat)+iepoch-1 &
           ,(acc_data(iepoch,axis,isat) -  (apriori(1,axis)*t**2+apriori(2,axis)*t) ,axis=1,3) &
           ,acc_data(iepoch,1:3,isat),(apriori(1,axis)*t**2+apriori(2,axis)*t+apriori(3,axis) ,axis=1,3)
      else
