...
! PT201023: add option to output just a vector of diagonal elements. Add flag "vector" for this at the end of the first header line.
! PT201214: add option to rescale based on the |amplitude| of the adjustment, rather than on the EWH value. This requires an additional command line argument.
! PT210103: add Patagonia as a region with its own sigma
! PT210205: add Ant_Pen to make the Antarctic Peninsula a region with its own sigma (it was part of Ant_ext)
! PT210205: add Totten as another separate region with its own sigma
! PT211015: change the W_Ant region definition to 85S across the whole region
! PT211018: modify the Ant_ext definition so that there is at least one 200 km mascon included around all the coastline (except W_Ant, Totten etc)
! PT211019: break W_Ant into W_Ant_coast and W_Ant_south
! PT221205: treat the Gulf of Bothnia the same as Hudson Bay (ie allow GIA uplift signal under the sea)
...
! variables to store the different sigmas to use in the regularisation
    ...
    real(kind=8) :: sigma_W_Ant_coast                ! for W Ant mass loss regions along the coast
    real(kind=8) :: sigma_W_Ant_south                ! for W Ant mass loss regions further south near the GIA uplift signal
    real(kind=8) :: sigma_Ant_Pen                    ! for Antarctic Peninsula mass loss regions
    real(kind=8) :: sigma_Totten                     ! for Totten region mass loss regions
    ...
    real(kind=8) :: scale_ocean_sigmas               ! PT201119: read from command file whether to scale ocean sigmas based on EWH value, or not 
...
! PT180831: variables for using mascon EWH values to scale off-diagonal correlations
    ...
    real(kind=8) :: msc_EWH_apr(50000)    ! we should never have more than 50000 mascons !
    integer*4    :: n_msc
    character*150:: line
    real(kind=8) :: scale_sigmas(3)   ! max EWH value and max scaling value
    real(kind=8) :: msc_EWH_tmp       ! temporary storage of a mascon EWH value
! PT201214: command line argument whether to use EWH value or adjustment value to rescale sigmas
    character*3  :: adjust_value
...
! PT201023: add flag for just writing out a vector of diagonal elements
    real(kind=8) :: vector
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! PT201214: 5th command line argument is whether to use the EWH value or the EWH adjustment
    call getarg(5,adjust_value)
    if(adjust_value == "adj")call status_update('STATUS','regularise_mascons ','regularise_mascons',' ' &
                              ,"Use adjustments to EWH, rather than total EWH, to rescale mascon sigmas",ioerr)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! first, the sigmas for the typical mascon regions
  ...
  call command_storecommand(lucmd,'sigma_W_Ant_coast'    ,sigma_W_Ant_coast,1,1,"West Ant coast     sigma",1)
  call command_storecommand(lucmd,'sigma_W_Ant_south'    ,sigma_W_Ant_south,1,1,"West Ant south     sigma",1)
  ...
  call command_storecommand(lucmd,'sigma_Antarctic_Pen'  ,sigma_Ant_Pen    ,1,1,"Antarctic Peninsul sigma",1)
  call command_storecommand(lucmd,'sigma_Totten'         ,sigma_Totten     ,1,1,"Totten Region      sigma",1)
...
! Pt201023: do we want to output just the diagonal elements as a vector?
  vector = 0.d0
  call command_storecommand(lucmd,'output_diag_vector',vector  ,1,1,"Output Diagonal vector only",1)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!



!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! get command file information on scaling sigmas by EWH values from a vcv
  if(msc_soln)then
    call command_storecommand(lucmd,'scale_by_EWH',scale_sigmas(1)  ,1,1,"Scale sigmas by EWH",0)
    if(scale_sigmas(1) == 1)then
      call command_storecommand(lucmd,'scale_by_EWH',scale_sigmas(2)  ,2,2,"Scale sigmas by EWH",0)
      call command_storecommand(lucmd,'scale_by_EWH',scale_sigmas(3)  ,3,3,"Scale sigmas by EWH",0)
    endif
  endif


! PT201119: scale ocean sigmas or not?
  scale_ocean_sigmas = 0.d0
  if(msc_soln)then
    call command_storecommand(lucmd,'scale_ocean_sigmas',scale_ocean_sigmas  ,1,1,"Scale ocean sigmas by EWH",0)
  endif
  if(scale_ocean_sigmas > 0.d0)then
    call status_update('STATUS','UTIL','regularise_mascons',' ',"Ocean sigmas will be scaled according to EWH value",0)
  else
    call status_update('STATUS','UTIL','regularise_mascons',' ',"Ocean sigmas will NOT be scaled according to EWH value",0)
  endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!    call status_update('STATUS','regularise_mascons ','regularise_mascons',' ',message,0)
  ...
  call status_update('STATUS','regularise_mascons ','regularise_mascons',' ',message,0)
  write(message,'(a,e11.3,a)') "Sigma on W_Ant_coast     mascons : ",sigma_W_Ant_coast," metres"
  call status_update('STATUS','regularise_mascons ','regularise_mascons',' ',message,0)
  write(message,'(a,e11.3,a)') "Sigma on W_Ant_south     mascons : ",sigma_W_Ant_south," metres"
  call status_update('STATUS','regularise_mascons ','regularise_mascons',' ',message,0)
  write(message,'(a,e11.3,a)') "Sigma on Ant Peninsula   mascons : ",sigma_Ant_Pen," metres"
  call status_update('STATUS','regularise_mascons ','regularise_mascons',' ',message,0)
  write(message,'(a,e11.3,a)') "Sigma on Totten region   mascons : ",sigma_Totten," metres"
  call status_update('STATUS','regularise_mascons ','regularise_mascons',' ',message,0)
...
2001  write(message,'(a,i6,a)')"Have read",n_msc," mascon EWH values"
    ...
    else if (line(1:17) == "V2 ADDNORM    VCV")then   ! it is an addnorm .vcv file
      ...
      do while(line(7:9) == " MC")
        ...
        if(line(7:9) == " MC")then
          n_msc = n_msc + 1
!print*,line(35:65)
          read(line(35:65),*)msc_ewh_apr(n_msc),msc_ewh(n_msc)
          if(adjust_value == "adj")then
!print*,n_msc,msc_ewh(n_msc) , msc_ewh_apr(n_msc)
            msc_ewh(n_msc) = msc_ewh(n_msc) - msc_ewh_apr(n_msc)
          endif
        endif
      enddo
...
! DEBUG:
if (i > 12750)print*,'mascon',i,' density',mcon_prim(i,6)
    if(mcon_prim(i,6) > 3000.0)then
      ...
      msc_label(i) = "GIA  "
        write(message,'(a,i7,2f9.4,a,f10.3)')'Setting  mascon',i,lon_prime(i)*180.0/pi,lat_prime(i) &
                                            ,' to GIA uncertainty of',sigma_Ant_GIA
        !call status_update('STATUS','UTIL','regularise_mascons',' ',message,0)
...
! PT180808: apply the density test to ensure that the uncertainty is applied only to land mascons
    if(lat_prime(i) > 60.d0 .and. lat_prime(i) < 74.d0 .and. lon_prime(i)*180.0/pi > 300.0 .and. lon_prime(i)*180.0/pi < 338.5d0 &
      ...
      msc_label(i) = "Grnld"
    else if(lat_prime(i)>74.d0 .and. lat_prime(i)<79.5d0 .and. lon_prime(i)*180.0/pi>287.5d0 .and. lon_prime(i)*180.0/pi<355.d0 &
            .and. mcon_prim(i,6) < 1020.d0 ) then   ! we are in northern Greenland
      sigma_msc(i) = sigma_Grn
      msc_label(i) = "Grnld"
! PT211020: need a complicated set of boxes to follow the strait between NW Greenland and Ellesmere Island
    else if(lat_prime(i)>79.5d0 .and. lat_prime(i)<81.25d0 .and. lon_prime(i)*180.0/pi>297.d0 .and. lon_prime(i)*180.0/pi<355.d0 &
            .and. mcon_prim(i,6) < 1020.d0 ) then   ! we are in northern Greenland
      sigma_msc(i) = sigma_Grn
      msc_label(i) = "Grnld"
    else if(lat_prime(i)>81.25d0 .and. lat_prime(i)<85.d0 .and. lon_prime(i)*180.0/pi>299.d0 .and. lon_prime(i)*180.0/pi<355.d0 &
            .and. mcon_prim(i,6) < 1020.d0 ) then   ! we are in northern Greenland
      sigma_msc(i) = sigma_Grn
      msc_label(i) = "Grnld"
    endif
...
! PT190507: now apply a different sigma for Antarctic oceans
      if(lat_prime(i) < -60.0 .and. mcon_prim(i,6) < 1020.d0)then
        ...
        ! PT210205: increase the southern latitude from S78 to S77 but add in the Pine Is and Thwaites mascons
        !if( (mcon_prim(i,2)*180.d0/pi > 200.d0 .and. mcon_prim(i,2)*180.d0/pi < 282.d0 .and. lat_prime(i) > -77.0) &
        !   .or. mcon_region(i)(1:4) == "Pine" .or. mcon_region(i)(1:6) == "Thwait") then
!print*,i,' ',mcon_prim(i,1)*180.d0/pi,mcon_prim(i,2)*180.d0/pi,mcon_region(i)
!          sigma_msc(i) = sigma_W_Ant
        if(mcon_prim(i,2)*180.d0/pi > 200.d0 .and. mcon_prim(i,2)*180.d0/pi < 282.d0 .and. lat_prime(i) > -79.d0) then
          sigma_msc(i) = sigma_W_Ant_coast
        else if(mcon_prim(i,2)*180.d0/pi > 200.d0 .and. mcon_prim(i,2)*180.d0/pi < 282.d0 .and. lat_prime(i) > -85.d0) then
          sigma_msc(i) = sigma_W_Ant_south

        ! PT210205: add the Antarctic Peninsula as a separate region
        else if(mcon_prim(i,2)*180.d0/pi >= 282.d0 .and. mcon_prim(i,2)*180.d0/pi < 305.d0 .and. lat_prime(i) > -77.0)then
          sigma_msc(i) = sigma_Ant_Pen

        ! PT210205: add the Totten region as a separate region
        else if(mcon_prim(i,2)*180.d0/pi >= 109.5d0 .and. mcon_prim(i,2)*180.d0/pi < 120.d0 .and. lat_prime(i) > -70.0)then
          sigma_msc(i) = sigma_Totten
        else if(mcon_prim(i,2)*180.d0/pi >= 109.5d0 .and. mcon_prim(i,2)*180.d0/pi < 120.d0 .and. lat_prime(i) > -72.5)then
...
! PT211018: need a more careful definition here so that E_Ant and just east of the Weddell Sea is also captured into Ant_ext
        else if (mcon_prim(i,2)*180.d0/pi > 0.d0 .and. mcon_prim(i,2)*180.d0/pi < 40.d0 .and. lat_prime(i) > -73.0)then
          sigma_msc(i) = sigma_Ant_ext
        else if (mcon_prim(i,2)*180.d0/pi > 40.d0 .and. mcon_prim(i,2)*180.d0/pi < 42.0d0 .and. lat_prime(i) > -71.0)then
           sigma_msc(i) = sigma_Ant_ext

        else if (mcon_prim(i,2)*180.d0/pi > 42.d0 .and. mcon_prim(i,2)*180.d0/pi < 63.9d0 .and. lat_prime(i) > -70.0)then
           sigma_msc(i) = sigma_Ant_ext

        else if (mcon_prim(i,2)*180.d0/pi > 63.9d0 .and. mcon_prim(i,2)*180.d0/pi < 76.2d0 .and. lat_prime(i) > -75.0)then
           sigma_msc(i) = sigma_Ant_ext
        else if (mcon_prim(i,2)*180.d0/pi > 76.2d0 .and. mcon_prim(i,2)*180.d0/pi < 150.d0 .and. lat_prime(i) > -70.0)then
           sigma_msc(i) = sigma_Ant_ext

        else if (mcon_prim(i,2)*180.d0/pi > 150.d0 .and. mcon_prim(i,2)*180.d0/pi < 166.d0 .and. lat_prime(i) > -85.5)then
           sigma_msc(i) = sigma_Ant_ext
        else if (mcon_prim(i,2)*180.d0/pi > 166.d0 .and. mcon_prim(i,2)*180.d0/pi < 310.d0 .and. lat_prime(i) > -87.0)then
           sigma_msc(i) = sigma_Ant_ext
        else if (mcon_prim(i,2)*180.d0/pi > 314.d0 .and. mcon_prim(i,2)*180.d0/pi < 348.d0 .and. lat_prime(i) > -76.5)then
           sigma_msc(i) = sigma_Ant_ext
        else if (mcon_prim(i,2)*180.d0/pi > 348.d0 .and. mcon_prim(i,2)*180.d0/pi < 350.d0 .and. lat_prime(i) > -73.5)then
           sigma_msc(i) = sigma_Ant_ext
        else if (mcon_prim(i,2)*180.d0/pi > 314.d0 .and. mcon_prim(i,2)*180.d0/pi < 360.d0 .and. lat_prime(i) > -73.0)then
           sigma_msc(i) = sigma_Ant_ext
...
! PT200319: brute force testing to find error in the Arctic ...
      if( lat_prime(i) > 58.0 .and.      lon_prime(i)*180.d0/pi > -1.d0 .and. lon_prime(i)*180.d0/pi < 361.d0    &
          .and. mcon_prim(i,6) >= 1020.d0 .and. mcon_prim(i,6) < 3000.d0)then
!print*,"constrain Arctic mascon",i,' by ',sigma_Arctic_o, lat_prime(i),lon_prime(i)*180.d0/pi
        sigma_msc(i) = sigma_Arctic_o
        msc_label(i) = "Arc_o"
      else if (lat_prime(i) > 60.0 .and. mcon_prim(i,6) >= 1020.d0  .and. mcon_prim(i,6) < 3000.d0)then
        sigma_msc(i) = 1.d3
...
! PT190905: increase the sigma of mascons seaward of Pine Island glacier, where there is 8 m EWH increase over GRACE era
        ...
        write(message,'(a,i7,2f9.4,a,f10.3)')'Setting  mascon',i,lon_prime(i)*180.0/pi,lat_prime(i) &
                                            ,' to 1 x W_Ant_coast uncertainty:',sigma_W_Ant_coast*1.d0
        call status_update('STATUS','UTIL','regularise_mascons',' ',message,0)
          sigma_msc(i) = sigma_W_Ant_coast * 1.d0
        endif
...
!      if(prim_flags(i)(1:5) == "PAmaz")then
      if(mcon_region(i)(1:6) == "AMAZON" .or. mcon_region(i)(1:6) == "Amazon" )then
        write(message,'(a,i7,2f9.4,a,f10.3)')'Setting  mascon',i,lon_prime(i)*180.0/pi,lat_prime(i) &
...
! PT200110: Baffin Island mascons. Define by one rectangle
      if(lon_prime(i)*180.0/pi > 278.5d0 .and. lon_prime(i)*180.0/pi < 298.8d0 &
        .and. lat_prime(i) > 65.9d0 .and. lat_prime(i) < 73.7d0  &
        .and. mcon_prim(i,6) < 1020.d0)then
        write(message,'(a,i7,2f9.4,a,f10.3)')'Setting  mascon',i,lon_prime(i)*180.0/pi,lat_prime(i) &
...
!           is not damped out as though it is ocean
    if( ((lat_prime(i)>51.d0 .and. lat_prime(i)<67.d0 .and. mcon_prim(i,2)*180.d0/pi>265.d0 .and. mcon_prim(i,2)*180.d0/pi<=277.d0) &
      .or. (lat_prime(i)>51.d0.and.lat_prime(i)<70.5d0.and.mcon_prim(i,2)*180.d0/pi>277.d0.and.mcon_prim(i,2)*180.d0/pi<294.5d0)) &
             .and. mcon_prim(i,6) >= 1020.d0 .and. mcon_prim(i,6) < 3000.d0)then
      sigma_msc(i) = sigma_land
...
! PT2212053: special case of Gulf of Bothnia. Change the density to treat it as "land" so that the GIA signal 
!            is not damped out as though it is ocean
    if(  (lat_prime(i)>53.d0 .and. lat_prime(i)<71.5d0 .and. mcon_prim(i,2)*180.d0/pi>3.d0 .and. mcon_prim(i,2)*180.d0/pi<=30.d0) &
             .and. mcon_prim(i,6) >= 1020.d0)then
      sigma_msc(i) = sigma_land
      mcon_prim(i,6) = 1000.d0
      msc_label(i) = "Bothnia"
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! PT201120: use a linearly varying scale of upscaling the input sigmas, based on the values found in the regularisation command file.
!           The minimum EWH will be considered to be sigma_msc, the maximum as read from the command flie (and stored in scale_sigmas(2)
!           The minimum scale factor is 1.0 - at value of sigma_msc, and will increase to a max of scale_sigmas(3)
      if(msc_soln .and. scale_sigmas(1) > 0.d0 .and. (mcon_prim(i,6) < 1020.d0 .or. scale_ocean_sigmas > 0.d0))then
        msc_EWH_tmp = dabs(msc_ewh(i))
        if(dabs(msc_ewh(i)) < sigma_msc(i))    msc_EWH_tmp = sigma_msc(i)
        if(dabs(msc_ewh(i)) > scale_sigmas(2)) msc_EWH_tmp = scale_sigmas(2)

! PT201222: kluge to stop this upscaling on the Caspian mascons. Instead, hardwire them to a particular value
!        if(i >= 1847 .and. i<= 1855)then
!          sigma_msc(i) = sigma_msc(i)
!        else
          sigma_msc(i) = sigma_msc(i) * (1.d0 + (scale_sigmas(3)-1.d0)*  (msc_EWH_tmp-sigma_msc(i) )/( scale_sigmas(2)-sigma_msc(i) ) )
!        endif
      endif
...
! PT201023: if we want just a vector of diagonal elements, add something to the first header line
    if(nint(vector) == 1)then
      call status_update('STATUS','UTIL','regularise_mascons',' ','Adding "vector" to end of first header line',0)
      msc_hdr_lines(1)(61:69) = "diag_only"
    endif
...
! write header lines containing the mascon file code plus info on how this constraint file was created
    ...
    write(luout,'(a,4f10.2,a,12e11.3,a)')'# scale dists land, ocean, Ant, Grn:',scale_land/1.d3,scale_ocean/1.d3 &
                  ,scale_Ant/1.d3,scale_Grn/1.d3 &
                  ,' km. Sigmas land/ocean/W_Ant_coast/W_Ant_south/Ant_ext/Ant_int/Grn/Alaska/Ant_alt/Casp/Patagonia/Baffin: ' &
                  ,sigma_land,sigma_ocean,sigma_W_Ant_coast,sigma_W_Ant_south,sigma_Ant_ext,sigma_Ant_int,sigma_Grn,sigma_alaska &
                  ,sigma_Ant_alt,sigma_caspian &
...
! PT201023: write out either the entire matrix or just the diagonal elements
      if(nint(vector) == 1)then
        write(luout,*)msc_const(i,i)   ! DEBUG ,i,1.d0/dsqrt(msc_const(i,i))
      else
