...
!
! PT200212: redo the algorithm for reshaping. Identify a mascon (e.g. largest/smallest/random?) and its neighbours, then use the voronoi cell
!           algorithm to reshape them all. Repeat with a different mascon etc until happy with the pattern ....
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if(mascon_file(1:1) == " ")then
    call status_update('WARNING','UTIL','reshape_mascons',' ', &
                   'Runstring: reshape_mascons mascon_file mascon_file_out region max_area/tern rms_limit [n_untouched + msc codes]',0)
    call status_update('FATAL','UTIL','reshape_mascons',' ', &
...
!        read(line(20:40),*)rms_limit
        ...
        call status_update('STATUS','UTIL','reshape_mascons','mascon.polygons',message,0)
...
1000 continue
    ...
    if(npoints == -999)then
      ...
      call status_update('FATAL','UTIL','reshape_mascons','mascon.polygons',message,0)
    endif
...
! check the max_tern_per_prim
  ...
  enddo
! end of identifying ternarys within region of interest
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! RM190212: from the area of the region, work out how many new primary mascons we need and allocate arrays
  if(prim_to_tern)then
    ...
    n_reshaped = nint(region_area/req_area + 0.25)
print*,"area: ",region_area,req_area,region_area/req_area
  endif
...
! PT161123: can we reduce the second dimension to ntern_reshape/4 so that we won't exceed memorylimits?
  allocate(msc_tern_reshaped(n_reshaped,max_tern_per_prim,nvar_tern))
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if(prim_to_tern)then
    ...
    if(cross_0E)then
      ...
      do i=1,n_reshaped
        ...
        enddo
      enddo
    endif    ! end of code to output each ternary mascon as a primary mascon

  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! here, we reshape the ternarys into n_reshaped primaries !!
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  else
debug = .true.
    ! PT200212: we need a set of starting crystallisation points for the algorithm. Call these the Centres of Mass (CoM).
    !           1. Initialise them by selecting N random ternarys from the complete list
    !           2. Use the CoMs and the voronoi cell algorithm to identify all the ternarys that are closest to each CoM
    !           3. Order by increasing numbers of ternarys per primary
    !           4. Identify a particular mascon (lowest number of ternarys/highest number of ternarys/random)
    !           5. Find primary mascons within X km of the idenfied mascon
    !           6. Pool the ternarys of the identified and neighbouring primary mascons
    !           7. Use the existing voronoi cell algorithm subroutine (voronoi_reshape_v2) to reshape this subset of ternarys
    !           
    !           8. repeat 4-7 until happy with the resulting configuration. Convergence criteria could be:
    !              a) RMS of variations of terns/perimary from nominal is < some value
    !              b) changes in CoM coordinates are < some value
    !              c) combination of the two ?

    ! first, make an array of all the ternarys of the region
    allocate(tern_crds(ntern_reshape,nvar_tern))
    print*,'n_to_reshape and region_prims',ntern_reshape,n_to_reshape,region_prims(1:n_to_reshape),"xxx",region,"XXX"
    call make_tern_array(n_to_reshape,ntern_reshape,.false.,region_prims,cross_0E,tern_crds)

    ! make a set of a priori CoMs for the new primaries. Select them at random from the subset of ternarys
    write(message,'(a,i6,a)')'Need',n_reshaped,' seed primary mascons'
    call status_update('STATUS','UTIL','reshape_mascons',' ',message,0)
    ...
    do iprim=1, n_reshaped
      ...
      ! and assign the coord information to the primary mascon
      msc_reshaped_prims(iprim,1:3) = tern_crds(extra_prim,1:3)
    enddo

    ! now, assign each of the ternarys to the nearest primary CoM. This gives us our first cut at making the new primary mascon pattern
    call status_update('STATUS','UTIL','reshape_mascons',' ',"Assign ternarys to initial primary mascon centres",0)
    call assign_ternarys(ntern_reshape,n_reshaped,tern_crds,msc_reshaped_prims,msc_tern_reshaped)

    ! now determine the CoM for each of these new primary mascons
    ! no, actually just assume that they are the same as the nucleating values. We will only update once we change ternary configurations
!print*,'now do the CoMs'
!!$OMP PARALLEL DO private (iprim) &
!!$OMP             ,shared(n_reshaped,ntern_reshape,msc_tern_reshaped,msc_reshaped_prims)
!    do iprim = 1, n_reshaped
!!      call MASCON_calc_CoM(max_tern_per_prim ,nvar_prim,nvar_tern,msc_tern_reshaped(iprim,:,:),msc_reshaped_prims(iprim,:) )
...
!!$OMP END PARALLEL DO
    ...
    ! now, call a subroutine to loop over the new primaries until happy with their collective shapes ....
    call jiggle_mascons(region,rms_limit,cross_0E,ntern_reshape,n_reshaped,msc_reshaped_prims,msc_tern_reshaped)
    ...
    !call voronoi_reshape_v2(.false.,region,rms_limit,cross_0E,n_to_reshape,region_prims_reshape,msc_reshaped_prims,n_reshaped &
...
! add a new line to the header to represent what has been done in running this program
  ...
  write(msc_hdr_lines(n_hdr_lines+1),'(a,a,a,i6,a,e12.6,a,1x,a8,1x,a10,1x,a5)') &
       "# reshape_mascons, region: ",region," has ",n_reshaped &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  call status_update('STATUS','UTIL','reshape_mascons',msc_file_out,message,0)
  nmsc_current = 0
  do imsc = 1,total_prim 
