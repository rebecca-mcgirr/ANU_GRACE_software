...
!PT170607: need to break the "ocean" into north and south of the equator - otherwise we hit memory errors
        ...
        prim_flags(imsc) = "P-----"
                                               ...
                                               .and. .not. leave_alone   )then      ! all the Southern Ocean ones
        n_to_reshape  = n_to_reshape  + 1
        region_prims(n_to_reshape ) = imsc
        ntern_reshape = ntern_reshape + nint(mcon_prim(imsc,8))
        region_area = region_area + sum(mcon_tern(imsc,1:nint(mcon_prim(imsc,8)),4))
...
! PT180320: decrease the latitude down to below Greenland (just under 60N)
                 ...
                 .and. prim_flags(imsc) /= "PLand ")then  ! all ocean mascons north of N65 and north of 58N around Greenland
        n_to_reshape  = n_to_reshape  + 1
        region_prims(n_to_reshape ) = imsc
        ntern_reshape = ntern_reshape + nint(mcon_prim(imsc,8))
        region_area = region_area + sum(mcon_tern(imsc,1:nint(mcon_prim(imsc,8)),4))
...
! RM190212: reshape primaries to specified area, send to voronoi reshape subroutine
  else
    call voronoi_reshape(region,rms_limit,cross_0E,n_to_reshape,region_prims_reshape,msc_reshaped_prims,n_reshaped &
                        ,msc_tern_reshaped,nint(ntern_reshape/1.0),debug,debug_rms)

! RM190518: force all densities to equal 3300 and land to equal 0, overwiting whats done in mascon_percent_land and calc_mascon_CoM
    do imsc=1,n_reshaped
      if( tern_flag(nint(msc_tern_reshaped(imsc,1,7)))(2:6) == "GIA" )then
        msc_reshaped_prims(imsc,11) = 0.d0
        msc_reshaped_prims(imsc,6) = 3300.d0
      endif
    enddo
  endif
