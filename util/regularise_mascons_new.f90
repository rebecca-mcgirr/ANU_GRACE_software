...
! JP191107: add a special case for Australia (sigma in cmd file)
...
! first, the sigmas for the typical mascon regions
  ...
  call command_storecommand(lucmd,'sigma_W_Antarctica'   ,sigma_W_Ant    ,1,1,"Antarctic exterior sigma",1)
...
! Australian mascons
  call command_storecommand(lucmd,'sigma_Australia'        ,sigma_Aus  ,1,1,"Australia sigma     ",1)  
...
! PT190507: Arctic Ocean mascons now have a different sigma. Define as > 58 N
      if(lat_prime(i) > 58.0 .and. mcon_prim(i,6) >= 1020.d0)then
...
! PT190719: set a different ocean constraint around the Argentine Gyre
      if(lon_prime(i)*180.0/pi > 300.d0 .and. lon_prime(i)*180.0/pi < 330.d0 &
        ...
        write(message,'(a,i7,2f9.4,a,f8.3)')'Setting  mascon',i,lon_prime(i)*180.0/pi,lat_prime(i) &
...
! JP191108: Australia
      if(prim_flags(i)(1:4) == "PAUS")then
        write(message,'(a,i7,2f9.4,a,f8.3)')'Setting  mascon',i,lon_prime(i)*180.0/pi,lat_prime(i) &
                                            ,' to Caspian Sea uncertainty of',sigma_Aus
        call status_update('STATUS','UTIL','regularise_mascons',' ',message,0)
...
! PT190920: Antarctic mascons where there is a large altimetry ice height change. For now, do this by primary mascon number in
!           mascons_stage4_V002
        if(sigma_Ant_Alt > 0.d0 )then
          ! clunky for now, but just hardwire the sigma for each mascon
          write(message,'(a,f8.3)')'Setting  mascons to sigma_Ant_Alt uncertainty:',sigma_Ant_Alt
          if(i == 1)call status_update('STATUS','UTIL','regularise_mascons',' ',message,0)
          sigma_msc(7429) = sigma_Ant_Alt
          sigma_msc(7227) = sigma_Ant_Alt
          sigma_msc(7306) = sigma_Ant_Alt
          !sigma_msc(8123) = sigma_Ant_Alt
          sigma_msc(8095) = sigma_Ant_Alt
          sigma_msc(6878) = sigma_Ant_Alt
          sigma_msc(6881) = sigma_Ant_Alt
          sigma_msc(7500) = sigma_Ant_Alt
          sigma_msc(6869) = sigma_Ant_Alt
          sigma_msc(6874) = sigma_Ant_Alt
          sigma_msc(6862) = sigma_Ant_Alt
          !sigma_msc(7228) = sigma_Ant_Alt
          sigma_msc(6873) = sigma_Ant_Alt
          sigma_msc(6872) = sigma_Ant_Alt
          !sigma_msc(7865) = sigma_Ant_Alt
          sigma_msc(8173) = sigma_Ant_Alt
        endif
...
! check whether we should be using the land or ocean scaling factors
          if ( mcon_prim(n,6) > 1000.d0 ) then
            ...
            scale = scale_ocean
            zero_correl = .true.   ! use this to turn on/off correlation between ocean mascons
          else
...
! write header lines containing the mascon file code plus info on how this constraint file was created
    ...
    write(luout,'(a,4f10.2,a,10e11.3,a)')'# scale dists land, ocean, Ant, Grn:',scale_land/1.d3,scale_ocean/1.d3 &
                  ,scale_Ant/1.d3,scale_Grn/1.d3 &
                  ,' km. Sigmas land/ocean/W_Ant/Ant_ext/Ant_int/Grn/Alaska/Ant_alt/Casp/Aus: ',sigma_land,sigma_ocean &
                  ,sigma_W_Ant,sigma_Ant_ext,sigma_Ant_int,sigma_Grn,sigma_alaska,sigma_Ant_alt,sigma_caspian,sigma_Aus,' m.'
    do i=1,total_prim
      if(mod(i,500) == 0 )then
