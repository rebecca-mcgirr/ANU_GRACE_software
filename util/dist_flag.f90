...
! PT200210: add option to just output a file of distances between ternary mascons for use in cluster_mascons.f90
...
! variables for calculating the distances between ternarys
  integer*4     :: ntern_dists
  real(kind=8),allocatable  :: ternary_dists(:,:,:)
...
! PT210826: just set this to 5,000 km
!  threshold(1) = 10000.d3
...
! decode runstring
  ...
  if(mascon_file(1:1) == " ")then
    write(message,'(a)')"Runstring: dist_flag input_mascon_file output_flag_file [info/ternary min_tern min_dist_sep]"
  ...
  if(info_flag == "info")then
    ...
    write(message,'(a,i4,a,f8.1)')"Will output only information about nearest primary mascons. Min ternarys: ",min_tern &
                                  ," Min CoM separation: ",min_dist_sep
    call status_update('STATUS','UTIL','dist_flag',mascon_file,message,0)
  else if (info_flag == "tern")then
    call status_update('STATUS','UTIL','dist_flag',mascon_file,"Will calculate the distances between ternary mascons",0)
...
! open the output file if required
  if (info_flag /= "info")then
    ...
    ! PT220106: don't write this out. It is the diagonal element for mascon 1. GRACEORB doesn't expect it to be there
    !write(9,'(i1,a)') 3, '  is this 1 1?'                     ! not sure what this one is ....  is it the flag for primary1 wrt primary1 ?
  endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! branch here to calculate and output in ascending order the distances between ternarys
  if(info_flag == "tern")then
    call status_update('STATUS','UTIL','dist_flag',mascon_file,"Calculate distances between ternarys",0)
    ntern_dists = 1000
    allocate(ternary_dists(max_tern,ntern_dists,2))
    call calc_tern_distances(ntern_dists,ternary_dists)
!    do itern1=1,max_tern
!      write(*,'(i7,5i7,5f15.1)')itern1,nint(ternary_dists(itern1,1:5,2)),ternary_dists(itern1,1:5,1)
!    enddo
    stop
  endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! PT181031: identify the closest pairs of primary mascons. Let's set a limit of less than 150km, then output all pairs within it
      if(base_dist < min_dist_sep*1.e3 .and. iprim2 /= iprim1)then
        write(message,'(a,f8.1,a,i6,i6,f15.3,a,2f8.1)')"Primary mascon centres of mass closer than ",min_dist_sep," km together: " &
...
! PT181102: only do this if we want the full running of dist_flag
      if(info_flag /= "info")then
        ...
        if ( base_dist .gt. threshold(1) ) then
          write(9,*) 1, counter,iprim1,iprim2 !,base_dist,' primary to primary exceeds threshold 1:',threshold(1)
          !if(debug)write(*,*) 1 ,base_dist,' primary',iprim2,' to primary',iprim1,' exceeds threshold 1:',threshold,counter
          if(debug .and. iprim2 > 11640)write(*,*) 1 ,base_dist,iprim1,iprim2,' to be treated as primaries'&
                         ,counter,1+(iprim2 - 1) + ((iprim1-1) * (iprim1 - 2))/2.d0
        else
          min_dist = base_dist
...
100       if ( min_dist .gt. threshold(2) ) then
            write(9,*) 1  !, counter,iprim1,iprim2 !,min_dist,' min_dist exceeds threshold(2)',threshold(2)
            if(debug)write(*,*) 1 !,min_dist,' min_dist exceeds threshold(2)',threshold(2)
          elseif ( min_dist .gt. threshold(3) ) then
            write(9,*) 2  !, counter,iprim1,iprim2 !,min_dist,' a secondary? ',threshold(3)
            if(debug)write(*,*) 2 !,min_dist,' a secondary? ',threshold(3)
          else
            write(9,*) 3  !, counter,iprim1,iprim2 !,min_dist,' must be a ternary'
            if(debug.and. iprim2 > 11640)write(*,*) 3 ,min_dist,iprim1,iprim2,' to be treated as ternaries' &
                         ,counter,1+(iprim2 - 1) + ((iprim1-1) * (iprim1 - 2))/2.d0
          endif
        endif
    ...
    if(info_flag /= "info")then
      ! PT220106: don't write out the diagonal element. GRACEORB code doesn't expect it to be there!
      !write(9,*) 3, iprim1,iprim2,' this one is from the end?' !,min_dist,' this one is from the end'
