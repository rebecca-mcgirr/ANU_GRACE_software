...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! RM190305: open the binary output file if required
  if (info_flag /= "info")then
    flag_rec = 15930192 
    open(unit=9, file=dist_flag_file, status='unknown',access='direct',recl=flag_rec)
    nrec=1
    write(9,rec=nrec) msc_hdr_lines(1)     ! write out the first header line of the mascon file, which contains the unique code for the mascon file
    nrec=nrec+1				  
    write(9,rec=nrec) total_prim           ! the second line is the total number of primary mascons
    nrec=nrec+1
    write(9,rec=nrec) 3			   ! not sure what this one is ....  is it the flag for primary1 wrt primary1 ?
  endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do iprim1 = 2,total_prim              ! total_prim assigned value when mascon file is read in read_mascon_file
...
! PT181031: identify the closest pairs of primary mascons. Let's set a limit of less than 150km, then output all pairs within it
      if(msc_to_msc_dist(iprim1,iprim2) < 150.e3 .and. iprim2 /= iprim1)then
...
! PT181102: output also if there are fewer than 100 ternary mascons in the primary
      if(mcon_prim(iprim1,8) < 100 .and. iprim1 == iprim2)then ! only print this out once per primary mascon
        write(message,'(a,i7,a,i3)')"Primary mascon ",iprim1," has fewer than 100 ternary mascons: ", nint(mcon_prim(iprim1,8))
        call status_update('STATUS','UTIL','dist_flag',mascon_file,message,0)
      endif
...
! PT181102: only do this if we want the full running of dist_flag
      if(info_flag /= "info")then
        if ( base_dist .gt. threshold(1) ) then
          nrec=nrec+1
          write(9,rec=nrec) 1 !,base_dist,' primary to primary exceeds threshold 1:',threshold(1)
...
100       if ( min_dist .gt. threshold(2) ) then
            nrec=nrec+1
            write(9,rec=nrec) 1 !,min_dist,' min_dist exceeds threshold(2)',threshold(2)
            if(debug)write(*,*) 1 !,min_dist,' min_dist exceeds threshold(2)',threshold(2)
          elseif ( min_dist .gt. threshold(3) ) then
            nrec=nrec+1
            write(9,rec=nrec) 2 !,min_dist,' a secondary? ',threshold(3)
            if(debug)write(*,*) 2 !,min_dist,' a secondary? ',threshold(3)
          else
            nrec=nrec+1
            write(9,rec=nrec) 3 !,min_dist,' must be a ternary'
    ...
    if(info_flag /= "info")then
      nrec=nrec+1
      write(9,rec=nrec) 3 !,min_dist,' this one is from the end'
