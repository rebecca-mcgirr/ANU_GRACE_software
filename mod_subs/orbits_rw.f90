...
contains
  ...
  subroutine orb_read_in_aMat(self, start_, count_, stride_, block_,  nepochs, partials, code)
    type(orbits_data), intent(in) :: self
    double precision, dimension(:,:) :: partials
    integer :: code  
    integer, intent(in) :: start_(2)
    integer, intent(in) :: count_(2)
    integer, intent(in) :: stride_(2)
    integer, intent(in) :: block_(2)
    integer, intent(in) :: nepochs

    integer(hsize_t) :: start_out(2)
    integer(hsize_t) :: count_out(2)
    integer(hsize_t) :: stride_out(2)
    integer(hsize_t) :: block_out(2)

    integer(hsize_t) :: dim_size(2), start_in(2)
    integer(hsize_t) :: data_dims(2) ! dimension in the memory
    ...
    !print *, code , btest(code, 0), btest(code, 1)
    if (btest(code, 0)) then        
      CALL h5dopen_f(self%file_id, "partials", dset_id, error)
    else if (btest(code, 1)) then
        CALL h5dopen_f(self%file_id, "msc_part", dset_id, error)
    endif
  ...
  !if (btest(code, 0)) then
      dim_size(1)=block_(1)*block_(2)
      dim_size(2) = 1
  !endif

  !print *, "dataset size ", dim_size
  !print *, "dataset size ", dim_size
  !print *, "memory size ", data_dims

  CALL h5screate_simple_f(2, data_dims, memspace, error)

  start_out(1)=start_(1)
  start_out(2)=start_(2)

  count_out(1)=count_(1)
  count_out(2)=count_(2)

  stride_out(1)=stride_(1)
  stride_out(2)=stride_(2)

  block_out(1)=block_(1)
  block_out(2)=block_(2)


  !print *,'++++ LOOP ++++'
  do idx = 1, nepochs
    start_in = (/0, idx-1 /) ! Initialise to the begining of the dataspace to read
    !print *, "Filespace start", start_in, "dim ",  dim_size 
    CALL h5sselect_hyperslab_f(dataspace, H5S_SELECT_SET_F, start_in, dim_size,  error)
    start_out(1)=start_(1) + (idx-1) * stride_(1)
   ! print *, "start " , start_out,  "block size", block_out 
    CALL h5sselect_hyperslab_f(memspace, H5S_SELECT_SET_F, start_out, block_out,  error)!, stride_out, block_out)
    CALL H5dread_f(dset_id, H5T_NATIVE_DOUBLE, partials, data_dims, error, memspace, dataspace)
  enddo



  end subroutine orb_read_in_aMat
...
!    print*, "output mat", data3d_dims
    ...
    start_in = (/0,0 /) ! Initialise to the begining of the dataspace to read
    CALL h5sselect_hyperslab_f(dataspace, H5S_SELECT_SET_F, start_in, dim_size,  error)
